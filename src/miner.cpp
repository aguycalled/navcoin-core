    ...
    if(IsCommunityFundEnabled(pindexPrev, chainparams.GetConsensus()))
    {
        ...
        for (unsigned int i = 0; i < vAddedProposalVotes.size(); i++)
        {
            ...
            if(CGovernance::FindProposal(vAddedProposalVotes[i].first, proposal))
            {
                if(proposal.CanVote() && votes.count(proposal.hash) == 0)
                {
                    ...
                    CGovernance::SetScriptForProposalVote(coinbaseTx.vout[coinbaseTx.vout.size()-1].scriptPubKey,proposal.hash, vote);
        ...
        for (unsigned int i = 0; i < vAddedPaymentRequestVotes.size(); i++)
        {
            ...
            if(CGovernance::FindPaymentRequest(vAddedPaymentRequestVotes[i].first, prequest))
            {
                ...
                {
                    coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
                    CGovernance::SetScriptForPaymentRequestVote(coinbaseTx.vout[coinbaseTx.vout.size()-1].scriptPubKey,prequest.hash, vote);
                    coinbaseTx.vout[coinbaseTx.vout.size()-1].nValue = 0;
        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CGovernance::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.hash == uint256())
                    continue;
                if(prequest.fState == CGovernance::ACCEPTED && prequest.paymenthash == uint256() &&
                        pindexPrev->nHeight - pblockindex->nHeight > Params().GetConsensus().nCommunityFundMinAge) {
                    CGovernance::CProposal proposal;
                    if(CGovernance::FindProposal(prequest.proposalhash, proposal)) {
                        ...
                        LogPrint("governance", "Could not find parent proposal of payment request %s.\n", prequest.hash.ToString());
                    }
                }
            }
        }
