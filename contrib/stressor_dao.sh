...
function start_node {
        $(echo $navpath)/navcoind -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -rpcuser=${array_user[$1]} -rpcpassword=${array_pwd[$1]} -devnet -daemon -debug=dao -ntpminmeasures=0 -staking=0 2> /dev/null
}
